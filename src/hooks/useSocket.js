...
export function useSocket({
  ...
  useEffect(() => {
    ...
    import('socket.io-client').then(({ io }) => {
      ...
      // Small delay to ensure server is ready (fixes ECONNREFUSED race condition)
  ...
  const trackOrder = useCallback((orderId, onStatusUpdateCb, onLocationCb) => {
    ...
    return () => {
      if (onStatusUpdateCb) socket.off('status-update', onStatusUpdateCb);
      if (onLocationCb) socket.off('delivery-location-update', onLocationCb);
    };
  }, []);
